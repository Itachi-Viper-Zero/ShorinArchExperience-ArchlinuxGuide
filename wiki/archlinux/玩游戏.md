...
8. 可选：软件默认是全屏打开，可以设置窗口化打开软件，F11 切换全屏和窗口化。
...
1. 确认 CPU 是否支持 `cpuid_fault`
    ...
    如果有输出，说明 CPU 支持 `cpuid_fault`，可以继续往下看，如果没输出说明不支持。通常 9 代 Intel / AM4（7000系）及以后的 CPU 会有这个功能。如果不支持的话需要编译内核模块，通过虚拟化技术模拟此功能。因为我没有设备无法测试，所以没有写入教程，有需要的话可以自己看原帖。
